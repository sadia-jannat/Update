...
def homedetails(request,pk):
    ...
    #recommandation code start..min and max 2000 less/high or 2 less/high
    #houserent filter
    min_range = fe.houserent-2000
    max_range = fe.houserent+2000
    
    #houseroom filter
    min_room=fe.bedroom-2
    max_room=fe.bedroom+2
    
    #bathroom filter
    min_bathroom=fe.bathroom-2
    max_bathroom=fe.bathroom+2
    #we can filter by also in house_list(bedroom__range=[min_room, max_room],bathroom__range=[min_bathroom, max_bathroom])
    #area filter
    area_name=fe.area
    
    #main house_list        
    global house_list
    house_list = ownerformfill.objects.filter(houserent__range=[min_range, max_range],area__icontains=area_name)
    
    #recommandation code end
    #rate and review code start
